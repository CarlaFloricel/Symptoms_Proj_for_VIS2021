...
export async function main() {
    ...
    window.addEventListener("load", async function () {
        ...
        var tip = d3tip().attr('class', 'd3-tip').html((d)=> d );
        svgg.call(tip)

        // tip.hide = function() {
        //     var nodel = d3.select(this)
        ...
        d3.selectAll(circles).on('mouseout', tip.hide) 
        ...
        d3.selectAll(circles)
            ...
            .on('mouseout', function () {
                // tip.hide()
                // Array.prototype.forEach.call(document.querySelectorAll('.d3-tip'), (t) => t.parentNode.hideChild(t));
                d3.select(this).style("stroke", "transparent")
                tip.attr("opacity", 0).style('pointer-events', 'none' )
            })
            .on("click", function () {

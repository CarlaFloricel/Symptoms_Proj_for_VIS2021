    ...
    window.addEventListener("load", async function () {
        ...
        d3.selectAll(texts2)
            ...
            .on('mouseout', function (event) {
                var e = d3.select(this)['_groups'][0][0].id
                var e2 = e.replace('_late_text', '')
                $(`#${e2}_late_symp`).css("stroke", "aliceblue")
                $(`#${e2}-highlight`).css("opacity", "0")
                $(`#${e2}`).css("color", "black")
            ...
            .on("click", function () {
                d3.select(this).classed("focused", !d3.select(this).classed("focused"))
                if (d3.select(this).classed("focused")) {
                    d3.selectAll(texts2).style("opacity", "0.2")
                    d3.selectAll(circles2).style("opacity", "0.2")
                    d3.selectAll(arrows2).style("opacity", "0.2")
                    d3.select(this).style("opacity", 1)

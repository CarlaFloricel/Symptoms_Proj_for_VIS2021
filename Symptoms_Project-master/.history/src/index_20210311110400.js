...
async sliderSupportUpdate(){
  ...
  if( window.oldSupValue > 1 || window.oldLiftValue > 1){
      ...
      const rules_late = data_late.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.008 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.008
      && parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.08 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.08)

...
console.log(window.oldSupValue)
console.log(window.oldLiftValue)
  if( window.oldSupValue > 1 || window.oldLiftValue > 1){
      ...
      if(! window.oldLiftValue > 1 && window.oldSupValue > 1){

        const rules_acute = data_acute.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.001 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.01)
       const rules_late = data_late.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.001 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.01)
      }
      else if(!window.oldSupValue > 1 && window.oldLiftValue > 1){
        const rules_acute = data_acute.filter(d => parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.007 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.09)
       const rules_late = data_late.filter(d =>  parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.08 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.08)
      }
      else{
        const rules_acute = data_acute.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.001 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.01
        &&  parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.007 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.09)
       const rules_late = data_late.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.001 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.01
       && parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.08 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.08)
      }
      ...
      console.log(rule_acute_rhs)
      console.log(rule_acute_lhs)
    })
    ...
    // $(`.graph_symptom_container`).css("opacity", "0.1")
  ...
  async drawTendrilPlot(patientIds, symptoms) {
    ...
    if ($("#show-mean-tendrils").is(":checked")){
      for (var i = 0; i < this.symptoms.length/2; i++){
        const tendrilPlot = new TendrilPlot('#tendril', 340, 300, [], this.symptoms[this.symptoms.length -i -1]);
        tendrilPlot.init();

...
const color = d3.scaleOrdinal(types, d3.schemeCategory10)
	...
	const li = await d3.csv('/data/mdasi_files/transaction_liks.csv');


	const data = ({n: Array.from(new Set(li.flatMap(l => [l.source, l.target])), node_name => ({node_name})), li})
	const links = data.li.map(d => Object.create(d));
	const nodes = data.n.map(d => Object.create(d));
  
	const simulation = d3.forceSimulation(nodes)
		.force("link", d3.forceLink(links).node_name(d => d.node_name))

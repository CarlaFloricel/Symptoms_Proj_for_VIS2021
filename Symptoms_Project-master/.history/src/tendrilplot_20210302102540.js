...
class TendrilPlot {
  ...
  init() {
    ...
    this.patientIdEl = this.svg
      ...
      .attr('font-size', '1rem')
      .attr('transform', `translate(${72},${250}), rotate(-90)`);
  ...
  drawTendrils(data, symptom, color) {
    ...
    'mood','enjoyment','activities', 'work', 'relations', 'walking'];
      ...
      const g = svg.append('g')
        ...
        .attr('transform', `translate(${width / 2},${height-90}) scale(7, 7)`)
    ...
    svg.attr("transform","translate(0, -90), rotate(90)")
    this.patientIdEl.text(symptom);
    const g = svg.append('g')
        .attr("class",'tendrils ' + symptom)
        //.classed('tendrils', true)
        .attr('transform', `translate(${width / 2},${height}) scale(4, 4)`)
          .on('mouseover', ()=>{
              var id =symptom;
    ...
    patients.forEach( (p,i) => {
      ...
      var time = 1;
        ...
        for(var k = 1; k< sum.length; k++){
            ...
            var angle = ((10+dif)/20) *  angleRange -  angleRange/2;
            const vala = rotate(0,0,0,15,-angle/(2*Math.PI)*360);
            prevX = vala[0] + prevX;
            prevY = vala[1] + prevY;
            points.push({x: prevX, y: prevY})

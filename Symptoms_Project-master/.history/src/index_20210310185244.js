...
async sliderSupportUpdate(){
  ...
  if( window.oldSupValue > 1 && window.oldLiftValue > 1){
      ...
      && parseFloat(d['lift']) >= parseFloat(window.liftlabels[window.oldLiftValue-1]) - 0.008 && parseFloat(d['lift']) <= parseFloat(window.liftlabels[window.oldLiftValue-1]) + 0.008)
  ...
  async showRules(event){
    ...
    var el = event.target.id
      if(el == 'Late'){
        ...
        window.lifttlabels =  ['2.1', "2.2", '2.3','2.8','2.9']
      ...
      if(el == 'Acute'){
        ...
        $('#RulesContainer').css('display', 'block')
        window.supportlabels = ['0.20', "0.21", '0.22','0.23','0.24']
        window.lifttlabels =  ['2.5', "2.54", '2.6','2.7','2.8']
        this.supportSlider.onChange();

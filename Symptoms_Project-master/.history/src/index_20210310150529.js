...
async sliderSupportUpdate(){
 console.log(window.currentSupport)
  if ($('#Acute').classed("active")) {


    const rule =acute_data.filter(d => d['support'] >= window.currentSupport-0.09 &&  d['support'] <= window.currentSupport+0.09)
  ...
  $(`.graph_circle`).css("opacity", "1")
    // $(`.graph_symptom_container`).css("opacity", "1")
    $(`.graph_symptom_text`).css("opacity", "1")
    $(`.graph_line`).css("opacity", "1").css("stroke", "#aaaaaa").css("stroke-width", '0.65')
} 
  ...
  async showRules(event){
    var el = event.target.id
      if(el == 'Late'){
        ...
        window.supportlabels = ['0.2', "0.21", '0.22','0.23', '0.26']
      ...
      if(el == 'Acute'){
        ...
        $('#RulesContainer2').hide()
        $('#RulesContainer').css('display', 'block')
        window.supportlabels = ['0.2', "0.21", '0.22','0.23','0.24']
        this.supportSlider.onChange();

...
async sliderSupportUpdate(){
  ...
  if( window.oldSupValue > 1){
    ...
    //    console.log("late")
    //  }
      const data = await d3.csv('/data/mdasi_files/acuteTopRules.csv');
    
      const rules_acute = data.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.008 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.008)
      const rule_acute_ids = rules_acute.map(e => e["rule_no"])
      const rule_acute_lhs = rules_acute.map(e => e["lhs"])[0].split(",")
      const rule_acute_rhs = rules_acute.map(e => e["rhs"])[0].split(",")
      console.log(rule_acute_ids)
      $(`.graph_symptom_text`).css("opacity", "0.1")
      $(`.graph_circle`).css("opacity", "0.1")
      $(`.graph_line`).css("opacity", "0.1")


      rules_acute_ids.forEach(id => {

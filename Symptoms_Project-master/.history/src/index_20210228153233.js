...
class App {
  ...
  async showPatientNeighbors(){
    ...
    if($("#show-patient-nearest_neighbors").is(":checked")){
      ...
      const data_patient = await d3.csv('/data/mdasi_files/mdasi_new_patients_features_with_therapy.csv');
  ...
  async onPatientFilter(period, redrawTendrils){
      ...
      if(this.patientFilters.includes("NoTherapy")){
        patients = (data.filter(d => d.therapeutic_combination != "Radiation alone" && 
         d.therapeutic_combination != "IC"  &&  d.therapeutic_combination != "CC"  &&
         d.therapeutic_combination != "IC+Radiation alone+CC" &&  d.therapeutic_combination != "IC+CC" &&
          d.therapeutic_combination != "CC+Radiation alone"  ));
         patients.forEach(el => therapyFilter.push(el));
  ...
  async loadDataset(period, symptoms, patientId) {
    ...
    const patients =await d3.csv('/data/mdasi_files/mdasi_new_patients_features_with_therapy.csv');
  ...
  async drawTendrilPlot(patientIds, symptoms) {
   ...
   // if (patientIds.includes(patient.patientId)
    if (this.filteredPatients.length> 1 && ! $("#show-patient").is(":checked") && ! $("#show-patient-nearest_neighbors").is(":checked")){
      ...
      for (var i = 0; i < this.symptoms.length; i++){
        const tendrilPlot = new TendrilPlot('#tendril', 140, 300, patientDataSelected,this.symptoms[this.symptoms.length -i -1],colors[i] );
        tendrilPlot.init();
      ...
      patients.forEach(patient => {
        ...
        if(this.tendrilPlots.length <4){
        const tendrilPlot = new TendrilPlot('#tendril', 140, 300, patientData);
        tendrilPlot.init();
        this.tendrilPlots.push(tendrilPlot);

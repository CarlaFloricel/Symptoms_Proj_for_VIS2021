  ...
  async drawTendrils(data, symptom) {
    ...
    if (!data.length) {
      ...
      if ($("#show-mean-tendrils").is(":checked")) {
        ...
        console.log(mean_tendril_data)
        var current_data_therapy_1 = mean_tendril_data.filter( d => {
         d['period'] == window.currentPeriod && d['therapeutic_combination'] == "Radiation alone"
        })
        var filtered_current_data_therapy_1 = current_data_therapy_1.map(el => {return el[symptom]})
        console.log(filtered_current_data_therapy_1)
      }

...
const color = d3.scaleOrdinal(types, d3.schemeCategory10)
	...
	const ni = await d3.csv('/data/mdasi_files/transaction_nodes.csv');
	const li = await d3.csv('/data/mdasi_files/transaction_links.csv');

// console.log(n)
	const data = ({ni: Array.from(new Set(li.flatMap(l => [l.source_node, l.target_node])), node_name => ({node_name})), li})
	const links = data.li.map(d => Object.create(d));
	const nodes = data.ni.map(d => Object.create(d));
	console.log(data)

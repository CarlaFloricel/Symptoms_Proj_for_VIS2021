  ...
  async drawTendrilPlot(patientIds, symptoms) {
    ...
    if ($("#show-mean-tendrils").is(":checked")){
      ...
      for (var i = 0; i < this.symptoms.length/2; i++){
        const final_mean = mean_patient.map(() => e[this.symptoms[this.symptoms.length -i -1]])
console.log(final_mean)
        const tendrilPlot = new TendrilPlot('#tendril', 340, 300, [], this.symptoms[this.symptoms.length -i -1], this.patients);
        tendrilPlot.init();

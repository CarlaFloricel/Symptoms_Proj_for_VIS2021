...
async sliderSupportUpdate(){
  ...
  if( window.oldSupValue > 1 || window.oldLiftValue > 1){
      ...
      const rules_acute = data_acute.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.01 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.01
      )

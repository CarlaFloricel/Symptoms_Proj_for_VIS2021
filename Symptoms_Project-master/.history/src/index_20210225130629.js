...
class App {
  ...
  async showPatientNeighbors(){
    ...
    if($("#show-patient-nearest_neighbors").is(":checked")){
      ...
      const data_patient = await d3.csv('/data/mdasi_files/mdasi_patients_with_outcome.csv');
  ...
  async loadDataset(period, symptoms, patientId) {
    ...
    const patients =await d3.csv('/data/mdasi_files/mdasi_patients_with_outcome.csv');
  ...
  const matrixData = await d3.csv(`/data/mdasi_files/mdasi_corr-nou-${window.currentPeriod}.csv`);
  console.loog(matrixData)
   for(var i = 0; i < 28; i++){

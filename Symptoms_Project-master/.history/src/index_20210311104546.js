...
class App {
  ...
  async initSupportSlider() {
    const supports = ['0.20', "0.21", '0.22','0.23','0.24']
  ...
  init() {
    ...
    window.liftlabels =  ['2.5', "2.54", '2.6','2.70.23','2.8']
    window.currentLift = 0.20
...
console.log(window.liftlabels[window.oldLiftValue-1])
  ...
  if( window.oldSupValue > 1 || window.oldLiftValue > 1){
      ...
      const rules_acute = data_acute.filter(d => parseFloat(d['support']) >= parseFloat(window.supportlabels[window.oldSupValue-1]) - 0.008 && parseFloat(d['support']) <= parseFloat(window.supportlabels[window.oldSupValue-1]) + 0.008

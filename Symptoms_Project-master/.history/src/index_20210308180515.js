...
class App {
  constructor() {
    ...
    this.patients = [];
    this.callTendrilDrawing = this.callTendrilDrawing.bind(this)
  ...
  init() {
    ...
    if ($("#show-mean-tendrils").on("click", this.callTendrilDrawing)){
...
async callTendrilDrawing(){
  ...
  async drawTendrilPlot(patientIds, symptoms) {console.log(this.symptoms)
    $("#RulesContainer2").hide();
    ...
    if ($("#show-mean-tendrils").is(":checked")){
      ...
      for (var i = 0; i < this.symptoms.length/2; i++){
        ...
        const tendrilPlot = new TendrilPlot('#tendril', 340, 300, this.symptoms[this.symptoms.length -i -1],colors[i] );
    }}
    else{

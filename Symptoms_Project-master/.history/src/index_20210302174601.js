    ...
    if (this.filteredPatients.length> 1 && ! $("#show-patient").is(":checked") && ! $("#show-patient-nearest_neighbors").is(":checked")){
      ...
      for (var i = 0; i < this.symptoms.length/2; i++){
        ...
        if ($("#show-colored-tendrils").is(":checked")){
           ...
           var ic = data_patient.filter(p => p.therapeutic_combination =='CC').map(patient => {return patient.patientId})
           ...
           var r = data_patient.filter(p => p.therapeutic_combination =='CC+Radiation alone').map(patient => {return patient.patientId})
            icr.forEach(p =>{

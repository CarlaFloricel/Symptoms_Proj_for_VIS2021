  ...
  async drawTendrils(data, symptom, mean_selection) {
    ...
    if (!data.length) {
      ...
      if ($("#show-mean-tendrils").is(":checked")) {
        ...
        // svg.attr("width", 100).attr("transform", "translate(120, -450), rotate(90)")

        // const g = svg.append('g')
        //   .classed('tendrils', true)
        //   .attr('transform', `translate(170,650) scale(2, 2)`)
      ...
      const g = svg.append('g')
        ...
        // this.patientIdEl.text(symptom).attr("font-size", '20');
        ...
        for (var i = 0; i < total_data.length; i++) {
          ...
          for (var k = 1; k < total_data[i].length; k++) {
            ...
            const vala = i < 8 ? rotate(0, 0, 0, 25, -angle / (2 * Math.PI) * 360) : rotate(0, 0, 0, 40, -angle / (2 * Math.PI) * 360);
            prevX = vala[0] + prevX;
            prevY = vala[1] + prevY;
            ...
            g.append('path')
              .attr('fill', 'none')
              .attr('class', "stackPath meanPath" +therapy_colors[i] )
              ...
              .on('mouseover', function () {
                ...
                // $('.mean_circle').css('opacity', '0.1');
                // $('.stackPath').css('opacity', '0.1');
                // // d3.select(this).style("opacity", 1).attr("stroke-width", "2px")
                // $(`#${therapy_colors[i]}tendril`).css("opacity", 1).css("stroke-width", "2px")
              })
              .on('mouseout', function () {
                tip.hide()
                // d3.select(this).attr("stroke-width", "0.5px")
                // $('.mean_circle').css('fill-opacity', '0.65');
                // $('.stackPath').css('opacity', '1');
         ...
         if( mean_selection && mean_selection.length >0){
          ...
          for (var k = 1; k < mean_selection.length; k++) {
            ...
            g.append('circle')
              ...
              .attr('fill', "red")
              .attr("class", "singlePatientCircle " + "circle")
              .attr("id", symptom + "circle")
      ...
      svg.attr("transform", "translate(100, -350), rotate(90)").style("margin-bottom", "100px !important")
